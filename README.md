...
It's Friday at 4:47 PM. You're staring at a pull request. 847 lines changed across 23 files. The CI is green — every single check passes. The commit messages are well-structured, the code reads clean, and it was generated in about twelve minutes by an agent that never gets tired, never gets distracted, and never second-guesses itself. You've been reviewing it for forty minutes. You understand maybe 60% of what it does. The backlog is stacking up. Your team lead pinged you twice. You type "LGTM," hit merge, and feel a knot in your stomach that you can't quite name.

Somewhere right now, a very confident AI is writing `UPDATE users SET role = 'admin'` and a very tired human is approving it at 4:47 PM on a Friday.

You didn't rubber-stamp it. Rubber-stamping is lazy — it's the act of not caring. This was something more anxious and visceral. You *tried* to review it. You opened every file, traced every function call you could follow, cross-referenced the ticket, checked the test assertions. And then you hit the wall of your own cognitive bandwidth. You approved it anyway, because the alternative was blocking the entire team for another day, and the product owner is already asking why the sprint velocity is "decelerating." You approved and prayed.

The numbers tell the story with uncomfortable precision. In 2025, [41% of code was already AI-generated](https://www.quantumrun.com/consulting/github-copilot-statistics/). By early 2026, teams using AI coding tools saw a [98% increase in PR volume with a 91% increase in review time](https://blog.logrocket.com/ai-coding-tools-shift-bottleneck-to-review/). Pull requests are [18% larger on average, and incidents per PR have climbed 24%](https://addyo.substack.com/p/code-review-in-the-age-of-ai). Senior engineers — the ones who are supposed to be the last line of defense — now spend an average of 4.3 minutes reviewing AI-generated code versus 1.2 minutes for human-written code. Why the discrepancy? Because AI code *looks* right even when it isn't, and the usual heuristics for spotting problems — weird indentation, unfamiliar patterns, suspiciously simple solutions — don't work. AI code is syntactically pristine. The bugs are semantic, structural, and deeply hidden. The [review bottleneck is already here](https://levelup.gitconnected.com/the-ai-code-review-bottleneck-is-already-here-most-teams-havent-noticed-1b75e96e6781), and most teams haven't noticed because the symptom looks like "going fast."

Here's the asymmetry nobody talks about openly: AI generation speed has increased roughly 10x in the last two years. Human comprehension speed has not increased at all. It can't. It's a biological constant — bounded by working memory, attention span, and the speed at which the human brain builds mental models of complex systems. We are connecting a firehose to a garden hose and wondering why the garden is flooding.

This is a backpressure problem. In distributed systems, when a producer overwhelms a consumer, backpressure builds until something breaks — the queue overflows, the consumer crashes, or data gets silently dropped. The same dynamics are playing out in engineering teams right now. Hyper-productivity generates backpressure on human reviewers. We became victims of our own implementation velocity. The agile board keeps moving — stories closed, velocity up, stakeholders happy — but the human pipeline is saturated. The people who need to *understand* the code are drowning in the volume of code they need to understand. And unlike distributed systems, there's no autoscaling for human cognition. You can't spin up another brain.

**Every approved-and-prayed PR permanently mints new cognitive debt. We are trading immediate delivery speed for long-term system comprehension, and the exchange rate is getting worse every quarter.**
